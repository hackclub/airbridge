...
router.get("/:base/:tableName", async (req, res, next) => {
  return res.status(501).send('v0.2 GET not implemented yet! Did you mean to use v0.1?')
...
router.post("/:base/:tableName", async (req, res, next) => {
  ...
  const tablePermission = Object.keys(res.locals.permissions[req.params.base]).includes(req.params.tableName)
  if (!tablePermission) {
    const error = new Error("Table not found or permissions insufficient")
    error.statusCode = 404
    return next(error)
  }
  const unpermittedFields = Object.keys(req.body).filter(field => !res.locals.permissions[req.params.base][req.params.tableName].post.includes(field))
  const fieldPermission = unpermittedFields.length === 0
  if (!fieldPermission) {
    const error = new Error(`Field(s) ${unpermittedFields.map(f => `'${f}'`).join(', ')} doesn't exist or permissions insufficient`)
  ...
  const createValues = Array.isArray(req.body) ? {fields: req.body} : [{fields: req.body}]
  const rawResults = await airinst
    .create(createValues)
    .catch((err) => console.log(err))
  ...
  const results = rawResults.map(rec => {
    ...
    const readPermissions = res.locals.permissions[req.params.base][req.params.tableName]?.get || []
    const writePermissions = res.locals.permissions[req.params.base][req.params.tableName].post

    const allowedFields = Array.from([...new Set([...readPermissions, ...writePermissions])])
